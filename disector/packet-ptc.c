...
void proto_register_ptc (void)
{
	static hf_register_info hf[] = {
		...
		{ &hf_ptc_seq,
		{ "SEQ Number", "ptc.seq", FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},
		
        { &hf_ptc_ack,
        { "ACK Number", "ptc.ack", FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},
	};
	static gint *ett[] = {
        &ett_ptc
	};
...
dissect_ptc(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	gint offset = 0;
	guint16 srcport, dstport, seq, ack;
	guint32 iflags;
	char flags[20] = "";
	...
	seq = tvb_get_ntohs(tvb, 8);
	ack = tvb_get_ntohs(tvb, 10);
	iflags = tvb_get_ntohl(tvb, 4);
	iflags = iflags >> 27;
	...
	if (check_col(pinfo->cinfo, COL_INFO)) {
		col_add_fstr(pinfo->cinfo, COL_INFO, "%d > %d [%s] [#SEQ: %d, #ACK: %d]",
	...
	if (tree) 
    {
        ...
        proto_tree_add_item(ptc_tree, hf_ptc_seq, tvb, offset, 2, ENC_BIG_ENDIAN);
        offset += 2;
        
        proto_tree_add_item(ptc_tree, hf_ptc_ack, tvb, offset, 2, ENC_BIG_ENDIAN);
	}
}	
